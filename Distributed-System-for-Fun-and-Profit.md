...
* [5 [复制技术: 弱一致性允许副本差异(accepting divergence)](http://book.mixu.net/distsys/eventual.html)](#5-[复制技术:-弱一致性允许副本差异(accepting-divergence)](http://bookmixunet/distsys/eventualhtml))
	* [5.1 弱一致性](#51-弱一致性)
...
- Scalability
    ...
    所以说，**everything starts with size - scalability**。在一个设计良好的分布式系统中，系统处理能力不会因为问题规模从小变大就变得很糟糕。
...
考虑到复制任务中的各个阶段，我们可以创建什么样的通信模式呢？我们选择的模式对性能和可用性有什么影响？
...
这个过程很直接明了，这里介绍了其过程中各个阶段的安排和通信方式，但是没有讨论同步阶段的算法细节信息。首先可以观察到，每个写操作需要通知所有节点完成同步并确认，然后才能返回响应给客户端。
...
后面会再稍微补充介绍，现在先来看下支持单副本一致性的复制算法。
...
P/B复制的同步版本能够确保在返回结果给client之前，数据不仅在master上存储了，也在其他从节点上存储了，开销就是同步等待时的时延边长了。需要注意的是，即便是这个变体，也只能提供若一致性保证，为什么呢？

考虑下这些场景，假定先写备份节点成功，再写主节点：

- 主节点收到了写请求，并且将该请求发送到了备份节点；
- 备份节点处理请求，持久化，并且ack当前写请求；
- 主节点再发送ack给client之前，却失败了；

这种情况下，client认为这次写操作失败了，但是实际上备份节点已经执行了当前写请求。如果备份节点被提升为主节点，就会出现错误。可能需要手动清理，来使失败的主节点、出现差异的从节点之间保持数据一致。

当然我们在这里做了很多简化。尽管所有P/B复制算法都遵循相同的常规消息传递模式，但是它们在故障转移、副本长时间脱机等方面的处理有所不同。但是，在这种方案中，不可能容忍主数据库的某些不当的故障。

在基于log-shipping或P/B复制的方案中，关键之处在于它们只能提供尽力而为的保证（例如，如果节点在不适当的时间失败，它们很容易丢失更新或更新不正确）。此外，P/B方案容易受到裂脑（split-brain）的影响，其中由于临时网络问题而导致的故障转移，从主节点转移到备份节点，并使主节点和备用节点同时处于活动状态。

为防止不适当的失败导致违反了一致性保证，我们需要添加另一轮消息传递，这使我们获得了两阶段提交协议（2PC）。

## 4.5 两阶段提交

两阶段提交（Two Phases Commit，简称2PC）在很多经典的关系型数据库中有使用，比如MySQL集群版（注意不是单机版的MySQL）就通过2PC提供了同步复制能力，下图中展示了消息交换的过程：
...
在第一阶段（voting，投票阶段）中，协调者（coordinator）发送更新请求给所有的参与者（participants），每一个participant处理这个更新请求，并且投票是否commit或者是否abort。当参与者投票表示可以提交的时候，参与者需要记录这次的更新到一个临时区域，如通过WAL日志（Write Ahead Log，预写日志)。在第二个阶段commit或者rollback之前，这里的更新都是临时的。

在第二阶段（decision，决策阶段）中，协调者决定这个结果，并且通知每一个参与者这个结果信息。如果所有的参与者都投票同意commit，那么这次commit对应的更新动作将从临时区域中拿出来并最终完成持久化。

在真正提交完成持久化之前增加上述第二个阶段是有用的，因为它允许系统遇到节点失败时可以回滚更新。相反，在一个主备复制（1PC）算法中，没有能力回滚一个部分节点成功、部分节点失败的操作，就会造成不同数据副本的发散、差异（diverge）。

2PC是倾向于阻塞的，因为一个节点失败（可能是参与者，也可能是协调者）会阻塞后续的处理过程，直到这个节点恢复。因为引入了上述第二个阶段，恢复也是可以实现的，在这个过程中，只要通知到其他没失败的节点rollback就行了。注意，2PC假定所有节点中的数据都是稳定存储、不会丢失的，并且也不会有节点一直crash无法恢复。但是真实情况是，节点出现故障后稳定存储中的数据也可能被破坏的。

关于节点故障后恢复过程（recovery procedure）的细节有点复杂，这里就先不展开了。主要的任务是确保对磁盘的写操作是持久化的（数据刷到磁盘，而非缓存到内存），并且要确保做出正确的恢复策略（就是说，协调者要能根据本轮操作的结果，决定是否本地重做或者撤销更新）。

在介绍CAP时，有提到过，2PC提交追求CA的复制算法，它不能容忍出现网络分区。2PC假定就不会出现网络分区这种情况，如果一个节点出现故障，预定的恢复方式就是不断等待直到节点恢复、分区情况自愈。如果一个协调者故障了，没有安全的方式能够提升一个节点成为协调者，因此需要手动干预。2PC对时延也敏感，因为它也是一个写N/N节点的方法，整体写操作受限于最慢回复ack的那个节点。

2PC在性能和容错方面达到了一种平衡，所以在关系型数据库场景中是比较常见的。然而，新系统常使用能容忍分区的共识算法，因为这种算法可以从临时的网络分区中自动恢复，节点间增加的时延也能实现更优雅的处理。

 下面来看下能够容忍网络分区的共识算法。

## 4.6 耐受网络分区的一致性算法

准备介绍分区耐受的共识算法，这些算法能帮助实现单副本一致性（single-copy consistency）。除了这一类分区耐受的共识算法，还有一类算法不仅能忍受网络分区，还可以忍受恶意节点攻击，这类算法就是能耐受拜占庭故障（byzantine faults）的共识算法。现实实现中，真正实现拜占庭容错的系统比较少，拜占庭容错实现也更复杂，运行起来开销也更高。所以这里先不介绍拜占庭容错的共识算法了，只介绍分区容错的共识算法。

>ps: 共识算法，就是一致性算法的意思，后面如没有引起歧义的地方，就不再强调了。文中可能会出现共识算法、一致性算法，请知悉，它们是一个意思。

最知名的分区容错的一致性算法，当属Paxos算法。但是Paxos背后的理论以及实现很复杂，也很难理解。而Raft则是一个专注于提高可理解性的分区容错的一致性算法，所以我们会聚焦在解释Raft一致性算法上。我们先来看下网络分区问题，以及分区容错的一致性算法的一些特点。

### 4.6.1 网络分区是什么？

网络分区指的是网络中一个或多个节点之间的网络链路故障，造成这些节点之间无法正常通信。那些正常的节点，依然可以接受来自相同分区一侧的客户端请求并处理。就像前面讲CAP理论时提到过的，网络分区确实会发生，但并不是所有的系统都能优雅的处理这种故障。

处理网路分区问题，是需要一点技巧的。因为无法区分远程的一个节点是出现了临时性的故障，还是节点不可达了。如果一个网络分区发生了，但是没有节点故障，那么这个系统聚会分化成两个分区，这两个分区将同时在线服务。下面两个图解释了网络分区和节点故障的差别。

一个有两个节点的系统，左图是节点故障，右图是网络分区：

![](images/system-of-2.png)

一个有三个节点的系统，左图是节点故障，右图是网络分区：

![](images/system-of-3.png)

一个保证单副本一致性（single-copy consistency）的系统必须有办法能够打破对称性，如果不能做到这点，在某些情况下就有可能会分裂为两个小系统，每个小系统各自服务进而会出现数据不一致的情况，也就违背了整个系统看上去是一个副本的理念。

保证单副本一致性的分区容错系统，要求系统在出现网络分区时只能有一个分区保持活跃、允许接收写操作，否则将无法阻止系统不同分区的数据发散。

### 4.6.2 网络分区应对策略

分区容错的一致性算法，依赖投票仲裁（收到大多数的投票）。要求节点中的大多数节点（而非2PC阶段中的所有节点）对更新数据达成共识，这能容忍少数节点出现故障、延时变慢、网络不可达。如果总节点数为N，只要保证`(N/2+1)`个节点正常就ok，整个系统依然可以正常运转。

分区容错的一致性算法，通常会让总节点数保持为一个奇数（如3，5，7）。如果只有两个节点，其中一个故障后，是不可能实现多数票投票仲裁的。例如，如果节点数为3，那么就能容忍一个节点故障。如果有5个节点，那就可以容忍2个节点出故障。

当系统出现网络分区时，不同分区必然是非对称的（节点数）。一个分区将包含总节点数中的大多数，这就是主分区（集群中加入了多少节点，各个节点是知道的吧，比如节点加入时可以gossip协议通知整个集群），其他的为次要分区。次要分区将停止处理请求以免出现数据发散的问题，主分区依然可以正常处理请求。这就确保了整个系统在任何时候都能保证单副本一致性。

多数投票也很有用，因为它们可以容忍分歧：如果出现扰动或故障，则节点的投票可能会有所不同。但是，由于只能有一个多数决定，因此暂时的分歧最多可以阻止协议继续进行（放弃liveness），但不能违反单副本一致性标准（safety）。

### 4.6.3 角色（Roles）

有两种方式来架构一个系统：系统中所有节点的角色可能是完全相同的，也可能会不相同。

用于复制的共识算法中，通常每个节点都具有不同的角色。拥有一个固定的leader服务器或master服务器是一项可使系统更高效的优化措施，所有更新操作都必须通过该leader服务器，非leader服务器仅需要将其请求转发给leader服务器即可。

请注意，具有独特的角色并不妨碍系统从领导者的失败中恢复（或任何其他角色）。 仅仅因为角色在正常操作期间是固定的，并不意味着一个人无法通过在失败后重新分配角色（例如通过领导者选举阶段）来从失败中恢复。 节点可以重复使用领导者选举的结果，直到发生节点故障和/或网络分区。

Paxos和Raft都使用不同的节点角色。特别是，它们具有一个leader节点（在Paxos中为提议者“proposer”），负责在正常操作期间进行协调。在正常运行期间，其余节点为跟随者（Paxos中的“接受者acceptors”或“投票者voters”）。

### 4.6.4 世代（Epochs）

Paxos和Raft中正常运行的每个时期都称为一个epoch（世代 or 纪元，Raft中称为任期“term”）。在每个任期，只有一个节点被指定为领导者。

![epoch](images/epoch.png)

选举成功后，同一领导者将协调直到时代结束。如上图所示（来自Raft论文），某些选举可能会失败，从而导致任期立即结束。

epoch充当逻辑时钟，当过时的节点何时开始通信时，其他节点可以通过epoch识别出来。已分区或无法运行的节点的epoch值会比当前节点的epoch数小，epoch数值小的命令将被忽略。

### 4.6.5 leader选举（leader changes via duels）

在正常操作期间，容忍分区的共识算法非常简单。 如前所述，如果我们不关心容错能力，则可以使用2PC。 实际上，大多数复杂性实际上来自于确保一旦做出共识决定，就不会丢失，并且协议可以处理由于网络或节点故障而导致的领导者变更。

所有节点都以跟随者的身份开始；一开始，一个节点被选为领导者。在正常操作期间，领导者会保持心跳，这使追随者可以检测到领导者是否失败或出现分区。

当节点检测到领导者已变得无响应（或在最初的情况下，不存在任何领导者）时，它会切换到中间状态（在Raft中称为“候选”），在此状态下，任期term/纪元epoch值会增加一个，发起领导人选举并竞争成为新领导人。

为了选出一个领导，一个节点必须得到多数选票。分配票数的一种方法是简单地以先到先得的方式分配票数。这样一来，一个领导者最终将被选出。为了避免多个节点同时发起新选举而引起的瓜分选票无法选出leader的情况，通常会给各个节点不同的超时时间，超时时间到了并且这段时间内没有收到leader的心跳，则当前节点会发起选举，因为超时时间是不同的，所以也就避免了同时发起选举而瓜分选票的情况。

### 4.6.7 提议编号（numbered proposals within an epoch）

在每个任期，leader一次针对一个value发起投票。在每个任期内，每个提案都以一个唯一的、严格递增的编号进行编号。followers（voters/acceptors）将接受特定提案编号的第一个proposal。

### 4.6.8 正常操作（normal operation）

在正常操作期间，所有提案都将通过leader节点。当client提交提案（例如更新操作）时，leader会联系系统中参与多数仲裁的所有节点。如果不存在任何竞争性提案（根据folloers的响应），则leader将建议采用该value。如果大多数followers接受该value，则认为该value已被接受，即达成共识。

由于可能另一个节点也试图充当leader，因此我们需要确保一旦接受了单个提议，其value就永远不会改变。否则，例如已被接受的提案可能会被竞争的负责人撤消。 

> Lamport表示为：
>
> P2: If a proposal with value v is chosen, then every higher-numbered proposal that is chosen has value v.

要确保拥有此属性，就需要算法限制followers和proposal都不能更改已被多数人接受的值。请注意，“值永远不会改变”是指协议的一次执行（或运行/实例/决策）的值。典型的复制算法将运行该算法的多次执行，但是对该算法的大多数讨论都集中在单次运行上，以使事情简单。我们要防止决策历史记录被更改或覆盖。

为了保证此属性，proposal必须首先询问followers，询问followers之前已经接收过的（编号最高的）提议和value。如果proposer发现提议已经存在，则它必须简单地完成协议的执行，而不是提出自己的提议。 
...
这是Paxos算法及其派生算法的核心。在协议的第二阶段之前，不会选择要提出的值。proposer有时必须简单地重传先前为确保安全而做出的决定（例如P2c中的b条款），直到他们知道自己可以自由施加自己的提案value（例如a条款）为止。

如果存在多个先前的提议，则提议编号最高的提议。如果根本没有相互竞争的提案，则proposer只需添加自己的提议值即可。

为了确保在propser向每个follower询问其最新提议值期间不会出现任何竞争性提议，proposer请follower不要接受其提议编号比当前提议编号小的提议。

将各个部分放在一起，使用Paxos做出决定需要两轮通信：
...
准备阶段使提议者可以了解任何竞争或先前的提议。第二阶段是提出新值或先前接受的值。在某些情况下-例如，如果两个提议者同时处于活动状态（决斗）； 如果消息丢失； 或如果大多数节点都发生了故障-那么大多数提议都不会接受任何提议。 但这是可以接受的，因为要建议的值的决策规则会收敛到单个值（在上一次尝试中建议值最高的那个）。

确实，根据FLP不可能结果，这是我们可以做的最好的事情：当无法达成有关消息传递范围的保证时，解决共识问题的算法必须放弃safety或liveness。Paxos放弃了liveness：它可能不得不无限期地推迟决策，直到没有竞争者领导的时间点，并且大多数节点都接受提案。 这比违反安全保证更为可取。

当然，实现此算法要比听起来困难得多。有许多小问题，即使在专家的掌握下，总的代码却相当多。 这些问题包括：

- 切合实际的优化
    - 通过leadership续约（而非heartbeat）代替重复的leadser选举；
    - 当处于稳定状态时，leader身份没什么变化，避免重复的提议信息；
- 确保followers和proposer不会在稳定存储中丢失信息，并且确保存储在稳定存储中的结果不会受到严重破坏（例如磁盘损坏）；
- 使集群成员资格能够以安全的方式更改（例如，基础Paxos取决于多数节点始终在一个节点上相交的事实，如果成员资格可以任意更改，则该节点不成立）；
...
如果一个系统不支持单副本一致性，会怎样？我们来看几个具体的例子。

可能，这类不支持单副本一致性的系统，最明显的特点就是允许不同副本的数据出现差异。这意味着没有严格定义的通信模式：副本数据可以出现差异，并且在出现差异之后仍然对外可以使用、接受读请求、接受写请求。

现在假定一个有3个副本的系统，其中每个副本都与其他的副本之间出现了分区。例如，这些副本位于不同的数据中心中，因为某些原因，它们之间无法进行通信了。每一个副本在分区期间都是可用的，并且能够接受客户端的读写请求。
...
一段时间之后，分区的问题恢复正常了，然后副本之间也可以交换信息了。因为在分区期间各个副本之间允许接收客户端读写请求，它们的数据之间已经出现了较大的差异，这个时候就需要一定的协调手段来解决这里的数据差异问题。我们希望发生的是所有副本都收敛到相同的结果。
...
换一个思路考虑下弱一致性保证的系统，假如有一些clients，这些clients按照一定的顺序发送消息给两个不同的副本。因为没有一个协调的协议来确保消息发送、接受按照一个全局的顺序，这些消息在两个副本收到的顺序可能是不同的，比如：
...
这就是我们提起“协调协议”的原因。例如，假定我们要连接一个字符串，待连接的字符串值位于3个消息的operation字段，这几个消息分别是：
...
如果没有协调的话，可能A连接出来是"Hello World!"，但是B连接出来是"World!Hello"。
...
这样的结果当然是错的了。重申，我们希望看到的是，这些副本A、B能够连接处相同的结果字符串，我们希望副本能结果一致。

我们先对这两个例子有印象，记在脑子里，带着这个简单的理解，我们先来看下Amazon Dynamo，将其作为一个基线（baseline），然后讨论下其他的一些构建弱一致性保证的方法，比如CRDT和CALM定理。

## 5.3 Amazon's Dynamo

Amazon's Dynamo系统设计（2007）可能是最知名的提供弱一致性保证+高可用的系统了。它是很多其他系统的基石，比如LinkIn的Voldemort、Facebook的Cassandra、Basho的Riak，等等。

Dynamo是一个追求最终一致、高可用的key-value存储系统，一个key-value存储系统就像一个大的哈希表（hash table）：一个客户端可以通过set(key,value)的方式在存储系统中存储一个键值对，然后通过get(key)的方式来获取键值。一个Dynamo集群包含了N个对等节点，每个节点都负责存储完整数据中的部分keys集合。

Dynamo在可用性和一致性方面，倾向于优先保证可用性，它不保证单副本一致性。对于数据写操作，它允许不同的副本出现差异。当读取数据时，如读取一个key时，会有一个协调的过程来尝试在不同的副本之间进行协调解决这里的数据不一致的问题，然后再把数据返回给客户端。

对于Amazon上的很多功能来说，避免服务中断，比保证完美的数据一致性更加重要，因为服务中断会导致很大的业务损失，也破坏用户信任。而且，如果数据一致性并不是非常重要，那么选择一个弱一致性的存储系统，相比强一致性或者RDBMS而言，能够以更小的代价换取更好的性能和更高的可用性。

因为Dynamo是一个完整的系统设计，除了其中的核心任务“复制”以外，还有很多其他的设计也值得学习。下图展示了Dynamo中的一些比较核心的任务，尤其是，一个数据写操作是如何路由到一个节点（node）的，以及又是如何写到其他的多个副本的（replicas）。
...
一个写操作被接受之后，我们再看下如何检测冲突，以及异步任务复制，这对实现高可用设计是有帮助的，节点总是可能会临时不可用（比如节点down机或者出现了分区），副本同步任务，能够确保节点在故障后恢复过程中迅速的同步数据 (catch up)。

## 5.4 一致性hash

不管是进行读操作、写操作，第一件事就是要计算，这个数据存储在系统中的哪个位置，需要在存储的键值（key）和存储的节点（node）之间做一层映射。

Dynamo中，使用一致性hash（consistent hashing），来将keys映射到节点。主要思想就是，客户端先通过key进行计算，计算出一组节点，然后将数据存储到这些节点上。一致性hash算法是很快的，这意味着客户端可以直接本地计算出这个key应该存储在何处，而不用rpc向后端发起请求或知存储的节点位置，效率上还是要好很多。

## 5.5 部分仲裁（少数投票）

当我们知道一个key应该存储在何处后，我们还需要做一些工作来完成数据的持久化。这就是一个同步的任务，我们之所以立即将value写到多个节点上，是为了提供一个更高的持久化保证，避免一个节点写后立即失败破坏持久。

就像Paxos和Raft一样，Dynamo使用投票来辅助实现复制，但是Dynamo的投票数是收到少数投票就可以，而并不是严格要求拿到多数人的投票。

部分仲裁（投票）不具有该属性，这意味着不需要多数，并且仲裁的不同子集可能包含相同数据的不同版本。用户可以选择要写入和读取的节点数：

- 用户可以选择N个节点中要写入W个成功才算成功 (W-of-N)；
- 用户可以选择N个节点中要读取R个节点 (R-of-N)；

W 和 R，指定了在一次写或者读中要参与的节点数量。写更多的节点，会让整个操作变得更慢，但是更能避免数据丢失，提高了持久化能力。从更多的节点进行读取也增加了读取到更新数据的可能性。

通常建议的数值是 `R + W > N`，因为这意味着参与读写的节点中，至少有一个同时参与了读、写，这有助于避免读取到一个更旧的值（stale value）。典型的配置是 N=3，一份数据有3个副本，用户可以选择的组合有：
...
- R=1, W=N，这种能够实现快速读取，但是写入很慢；
- R=N，W=1，这种能够实现快速写入，但是读取很慢；
- R=N/2，W=N/2+1，这种对于读、写都能有不错的性能；

副本数N通常不会超过3，因为保留太多副本数，成本也是不容小觑的。

前面提过，Dynamo论文启发了很多其他的类似设计，它们也使用了这里的“部分仲裁”的方案，但是为N、W、R使用了不同的默认值：

- Basho's Riak (N = 3, R = 2, W = 2 default)
- Linkedin's Voldemort (N = 2 or 3, R = 1, W = 1 default)
- Apache's Cassandra (N = 3, R = 1, W = 1 default)

还有另一点要提下：当发送一个读请求或者写请求的时候，Riak要求所有的N个节点都要做出响应，Voldemort要求部分节点做出响应（满足RWN约束即可）。这种“send-to-all“的方式只等最快的一个节点回包即可，所以比较快，对延时不那么敏感，能够避免个别慢速节点的影响，但是也不是很高效。另一种“send-to-minimum”的方式对于延时比较敏感，很可能因为收请求的节点的问题对延时造成影响，但是因为发送的消息更少了、建立连接负载更小了，也会高效点。

当`R + W > N`时，即读写节点数重写重叠时，会发生什么呢？通常声称这可以实现强一致性（strong consistency），注意只是声称！

ps：严格的仲裁系统，指的是读取、写入的节点集合R+W必须能够重叠，即`R + W > N`。

## 5.6 R+W>N？是否等同于强一致？

No !

一个系统如果能够保证 `R + W > N`，就能够检测到读写冲突，因为参与读仲裁、写仲裁的两个节点集合中，至少有一个节点同时参与了读、写。
...
这意味着之前的写操作，能够被后面的读操作观察到。然而，这只有在节点数N不改变的情况下才能保证。因此，Dynamo不符合条件，因为如果节点发生故障，Dynamo集群成员会发生变更。

Dynamo的设计是保证总是可写的，它引入了一种机制来处理节点故障，当某个节点故障之后，它会添加一个不同的、不相关的服务器到负责特定keys的节点集合中。这意味着仲裁不能够被保证总是会出现读写重叠。甚至是当`R=W=N`时也不符合，因为即便当投票人数等于N时，参与投票的节点会因为出故障而发生变化。

而且，Dynamo不能够像实现强一致性的系统那样处理分区问题：就是说，出现分区之后，分区两侧都能够同时处理写请求，至少会出现一段时间，分区两边的副本出现不一致，表现就是看起来不是一个单副本。所以说称`R + W > N`等同于实现强一致性是错误的，这里的声称或者保证仅仅是概率性的，并不总是成立。另外，强一致性也不是指的这个。

## 5.7 冲突检测 & 读修复

允许副本出现数据不一致的系统，需要采用一种方法来最终协调不同的值到一个值。在少数仲裁方法中有提到，可以在读取的时候实现冲突的检测，然后应用某些解决冲突的方法来解决冲突。但是具体该如何做呢？

一般，可以这样实现，跟踪数据的因果关系历史，怎么跟踪呢，为数据补充一些元数据信息。客户端从系统中读取数据时，客户端必须保存返回的元数据信息，向系统中写入数据时，也必须再将这些元数据信息带回。

我们遇到了一个这么做的方法：时钟向量可以用来表示一个值的历史。实际上，这就是以前版本的Dynamo所采用的方式，用这种方式来检测冲突。

然而，使用时钟向量并不是唯一的方式，如果你看过很多实际的系统设计，你就可以通过这些用于跟踪数据历史的元数据信息，来推断出到底如何工作的。

- No metadata

当一个系统没有跟踪数据的元数据信息，客户端查询时只是返回值的话，这样的系统对数据并发写操作是奈何不得的。通常是最后一个写的，换句话说，如果有两个writers同时写同一个数据，那么这个写的比较慢的writer会最终写成功。

- Timestamps

名义上，拥有更大的时间戳的值最终会被写入。然而，如果时间没有经过很好地同步，就会出现一些奇怪的事情，比如一个存有旧数据、但是时钟更快的系统，会覆盖一些拥有新数据但是时钟正常或者偏慢的系统中的数据。Facebook的Cassandra是一个Dynamo变体，它使用了时间戳代替向量时钟。

- Version numbers

版本号能够避免时间戳的问题，请注意，在可能存在多个历史记录时，能够准确跟踪因果关系的最小机制是向量时钟，而不是版本号。

- Vector clocks

使用向量时钟，并发更新请求、过期的更新请求（out of date）都能够被检测到。执行读取修复就变得可能了，尽管某些场景下（并发修改）我们需要让客户端决定一个值。这是因为，如果这个变更是并发的，我们对数据又一无所知（比如一个简单的key-value存储系统），比较好的方式就是询问决定用哪个值，而不是任意丢弃一个。

当客户端读取一个值时，客户端请求N个节点中的R个，然后比较R个节点的请求，并将老的数据丢弃（可以跟踪向量时钟，向量时钟中每个节点都有一个版本概念的数值在）。如果最后只有一个唯一的向量时钟和value pair，那么直接返回这个值。如果最后有多个向量时钟和多个value pairs（并发修改的，不可比较新旧），就全部返回，让客户端自己决定用哪个。

前面内容有说明，读取修复操作，依然可能会返回多个值。这意味着客户端/应用程序开发人员必须处理这类case，怎么处理呢，基于一些特定case的策略来从多个值中选择一个看上去更合理的。

此外，基于时钟向量构建的系统，需要关注一个点，就是这里的时钟向量不能无限制增长，需要考虑一种合理的方式来垃圾回收，来平衡容错和存储需求。
...
一定概率的过时(PBS)，更多的关于Dynamo设计的内容：

- 通过一致性hash（consistent hashing）来决定key的存放节点集合；
- 部分仲裁（partial quorums）用来实现读写；
- 冲突检测（conflict detection）用来进行读取修复（通过向量时钟、gossip）；
- gossip用来解决副本同步问题；

我们如何来归纳这样一个系统的特点？Bailis et al. (2012) 最近的一篇论文描述了这样一种方法，叫PBS (probabilistically bounded staleness)，这种方法通过模拟以及真实系统中收集的数据来描述这样的系统的期望的行为。

PBS，通过信息的anti-entropy (gossip) rate、网络延时和节点本地处理时延来估计副本不一致的程度，进而估计数据读取时能达到的一致性程度。Cassandra中已经实现了这种方法，Cassandra中，时序信息被附加在其他消息上，并在蒙特卡洛模拟中基于该信息的样本来计算估计值。

基于该论文，在正常操作期间，要求最终一致性的数据存储通常会更快，并且可以在数十或数百毫秒内读取到一致性的状态。下表说明了在LinkedIn（SSD和15k RPM磁盘）和Yammer的经验性时序数据上给定不同的R和W设置的情况下，对于99.9％的一致读取概率，所需的时间量：

![pbs](images/pbs.png)

例如，Yammer案例下从R=1,W=1到R=2,W=1，数据不一致窗口从1352ms减少到了202ms，而读取延时比最快的严格仲裁(R=3,W=1,219.27ms）要低（32.6ms)。

更详细信息，可以参考[PBS website](http://pbs.cs.berkeley.edu/)以及相关的论文。
...
CRDT数据结构基于以下认识：可表示为半格的数据结构是会聚的。 但是，编程不仅仅涉及不断发展的状态，除非您只是实现数据存储。

显然，顺序无关性是任何会收敛的计算的重要属性：如果接收数据项的顺序会影响计算结果，那么就无法在不保证顺序的情况下执行计算。

但是，在许多编程模型中，语句的顺序并不重要。 例如，在MapReduce模型中，将Map和Reduce任务都指定为需要在数据集上运行的无状态元组处理任务。 没有明确指定有关如何将数据路由到任务以及以什么顺序路由到任务的具体决策，而是由批处理作业调度程序负责调度要在集群上运行的任务。

同样，在SQL中，指定查询，但不指定查询的执行方式。 查询只是对任务的说明性描述，而查询优化器的工作就是找出有效的执行查询的方法（跨多台计算机，数据库和表）。

当然，这些编程模型不像通用编程语言那样宽松。 MapReduce任务需要在非循环数据流程序中表示为无状态任务。 SQL语句可以执行相当复杂的计算，但是其中很多事情很难表达。

但是，从这两个示例应该清楚的是，有许多种数据处理任务都可以用声明性语言来表达，而没有明确指定执行顺序。 表示期望结果同时将语句的确切顺序留给优化器决定的编程模型通常具有与顺序无关的语义。 这意味着这样的程序无需协调即可执行，因为它们取决于接收到的输入，但不一定取决于接收输入的特定顺序。

关键是此类程序无需协调即可安全执行。 如果没有明确的规则来刻画无需协调就可以安全执行的内容，而不是没有协调的内容，那么我们就无法在确保结果正确的前提下实施程序。

这就是CALM定理的目的。 CALM定理基于对逻辑单调性与最终一致性的有用形式（例如融合/收敛）之间的联系的认识。 它指出，逻辑上单调的程序可以保证最终是一致的。

然后，如果我们知道某些计算在逻辑上是单调的，那么我们知道在没有协调的情况下执行也是安全的。

为了更好地理解这一点，我们需要将单调逻辑（或单调计算）与[非单调逻辑](http://plato.stanford.edu/entries/logic-nonmonotonic/)（或非单调计算）进行对比。

> 单调 (Monotony) 如果句子φ是一组前提Γ的结果，则也可以从扩展Γ的任何前提Δ集合中推论得出

大多数标准逻辑框架都是单调的：在框架中进行的任何推论（例如一阶逻辑）一旦演绎有效，就不能被新信息所无效。 非单调逻辑是其中该属性不成立的系统-换句话说，如果某些结论可以通过学习新知识而无效。

在人工智能界，非单调逻辑与[不可行的推理](http://plato.stanford.edu/entries/reasoning-defeasible/)相关联，在推理中，利用部分信息做出的断言可以被新知识所无效。 例如，如果我们了解到Tweety是一只鸟，我们将假定Tweety会飞； 但是如果以后我们知道Tweety是企鹅，那么我们就必须修改我们的结论。

单调性涉及前提（或关于世界的事实）与结论（或关于世界的断言）之间的关系。 在单调逻辑中，我们知道我们的结果是无收缩的：单调计算不需要重新计算或协调；单调计算不需要重新计算或协调。 随着时间的推移，答案将变得更加准确。 一旦我们知道Tweety是一只鸟（并且我们正在使用单调逻辑进行推理），我们就可以安全地得出结论Tweety可以飞翔并且我们学到的任何东西都不能使该结论无效。

尽管任何产生面向人类结果的计算都可以解释为关于世界的断言（例如，“ foo”的值为“ bar”），但很难确定基于冯·诺依曼机器的编程模型中的计算是否为 单调的，因为尚不清楚事实和断言之间的关系是什么，以及这些关系是否单调。

但是，有许多编程模型可以确定单调性。 尤其是关系代数（例如SQL的理论基础）和数据记录提供了具有很好理解的解释的高表达语言。

基本的Datalog和关系代数（甚至具有递归）都被认为是单调的。 更具体地说，已知使用特定的一组基本运算符表示的计算是单调的（选择，投影，自然联接，叉积，并和递归数据日志，且不带求反），而非单调性是通过使用更高级的运算符引入的（求反， 设置差异，划分，通用量化，聚合）。

这意味着在这些系统中使用大量运算符（例如，地图，过滤器，联接，联合，交点）表示的计算在逻辑上是单调的； 使用这些运算符的任何计算也是单调的，因此无需协调即可安全运行。 另一方面，使用否定和聚合的表达式在没有协调的情况下运行是不安全的。

重要的是要实现非单调性与在分布式系统中执行昂贵的操作之间的连接。 具体而言，分布式聚集协议和协调协议都可以被视为否定的一种形式。 正如乔·海勒斯坦（Joe Hellerstein）所说：

> 为了在分布式环境中确定否定谓词的准确性，评估策略必须开始“计数到0”来确定空度，并等待直到分布式计数过程明确终止为止。 聚合就是这种想法的概括。

and

> 这个想法也可以从另一个方向看。 协调协议本身就是聚合，因为它们需要投票：两阶段提交需要一致投票，Paxos共识需要多数投票，拜占庭协议需要2/3多数。 等待需要计数。

如果能够以一种可以测试单调性的方式来表示我们的计算，那么我们就可以执行整个程序的静态分析，从而检测出程序的哪些部分最终是一致的并且可以在没有协调的情况下安全地运行（单调性）。 零件）-哪些零件不是（非单调的零件）。

请注意，这需要使用另一种语言，因为对于以序列，选择和迭代为核心的传统编程语言而言，很难得出这些推论。 这就是设计Bloom语言的原因。

## 5.13 什么是非单调性（non-mononicity）

单调性和非单调性之间的区别很有趣。 例如，将两个数字相加是单调的，但是在包含数字的两个节点上计算聚合不是一个单调的过程。 有什么不同？ 其中之一是计算（将两个数相加），而另一个是断言（计算聚合）。

计算与断言有何不同？ 让我们考虑一下查询“比萨饼是蔬菜吗？”。 要回答这个问题，我们需要深入了解：什么时候可以推断出某些事情是正确的（或不正确的）？

有几个可接受的答案，每个答案对应于关于我们所拥有的信息以及我们应采取的行动方式的一组不同的假设-我们已经在不同的情况下接受了不同的答案。

在日常推理中，我们进行所谓的“[开放世界](http://en.wikipedia.org/wiki/Open_world_assumption)”假设：我们假设我们并不了解一切，因此无法从缺乏知识中得出结论。 也就是说，任何句子都可以是正确的，错误的或未知的。
...
在做出开放世界假设时，我们只能安全地断言可以从已知事物中得出的东西。 我们关于世界的信息被认为是不完整的。

让我们首先看一下我们知道推理是单调的情况。 在这种情况下，我们拥有的任何（可能不完整的）知识都不会通过学习新知识而失效。 因此，如果我们根据某些推论推断出句子是正确的，例如“含有两汤匙番茄酱的东西是蔬菜”和“比萨饼含有两汤匙番茄酱”，那么我们可以得出结论：“比萨饼是一种蔬菜 ”。 如果我们可以推断出句子是错误的，那也是同样的道理。

但是，如果我们不能推断出任何东西-例如，我们拥有的知识集仅包含客户信息，而与披萨或蔬菜无关，则在开放世界假设下，我们不得不说我们无法得出任何结论。

有了非单调知识，我们现在所知道的任何信息都可能被无效化。 因此，即使我们可以从我们目前所知道的推论中得出对或错，我们也无法安全地得出任何结论。

但是，在数据库环境中以及在许多计算机科学应用程序中，我们希望得出更明确的结论。 这意味着假设所谓的封闭世界假设：任何无法证明为真的东西都是假的。 这意味着不需要显式的虚假声明。 换句话说，我们拥有的事实数据库被认为是完整的（最小的），因此其中任何不存在的事物都可以被认为是错误的。

例如，在CWA下，如果我们的数据库中没有旧金山和赫尔辛基之间的航班条目，那么我们可以安全地断定不存在此类航班。

我们还需要做一件事才能做出明确的断言：[逻辑上的限制](http://en.wikipedia.org/wiki/Circumscription_%28logic%29)。 割礼是一种正式的猜想规则。 领域限制猜想已知实体全都存在。 我们需要能够假设已知实体全部存在，以便得出确定的结论。
...
特别是，非单调推断需要此假设。 如果我们假设我们拥有完整的信息，那么我们只能做出自信的断言，因为否则其他信息可能会使我们的断言无效。

实际上这是什么意思？ 首先，单调逻辑一旦得出句子是正确的（或错误的）就可以得出确定的结论。 其次，非单调逻辑还需要一个额外的假设：已知实体就在那里。

那么，为什么表面等效的两个操作却不同呢？ 为什么将两个数字单调相加，而不是在两个节点上计算合计呢？ 因为聚合不仅计算总和，而且还断言它已经看到所有值。 唯一的保证方法是跨节点协调并确保执行计算的节点确实看到了系统中的所有值。

因此，为了处理非单调性，需要使用分布式协调来确保仅在所有信息已知之后才进行断言，或者需要做出断言，以便以后可以使结论无效。

出于表达性的原因，处理非单调性很重要。 这归结为能够表达非单调的事物。 例如，可以说某列的总数为X很好。系统必须检测到这种计算需要全局协调边界以确保我们已经看到所有实体。

纯单调系统是罕见的。 似乎大多数应用程序即使在数据不完整的情况下也可以在封闭世界的假设下运行，我们人类对此表示满意。 当数据库告诉您不存在旧金山和赫尔辛基之间的直航时，您可能会将其视为“根据该数据库，没有直航”，但您不排除在现实中这样的可能性 航班可能仍然存在。

实际上，仅当副本可以分开时（例如，在分区期间或由于正常操作期间的延迟），此问题才变得有趣。 然后，需要进行更具体的考虑：答案是仅基于当前节点，还是基于系统的整体。

此外，由于非单调性是由声明引起的，因此似乎许多计算可以长时间进行，并且仅在将某些结果或声明传递给第三者系统或最终用户时才进行协调，这似乎是合理的。 当然，如果系统中的每个读写操作只是长时间运行计算的一部分，则不必强制执行总顺序。
